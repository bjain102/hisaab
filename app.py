...
# Shortest merchant alias that may be stored. Alias matching is INSTR-based —
# a substring hit anywhere in a normalized description wins — so a very short
# alias is not a narrow rule, it is a wildcard. See _confirm_merchant.
# Deliberately stricter than normalize()'s own MIN_MERCHANT_LEN (3) — that one
# asks "is this a plausible identity at all", this one asks "is this specific
# enough to safely wildcard-match arbitrary future transactions". Different
# risk, different bar; not a drift between the two files.
MIN_ALIAS_LEN = 4
...
CATEGORY_RULES = {
    ...
    "Credit Card Bills": [
        ...
        "hdfc cc", "icici cc", "axis cc", "kotak cc", "idfc cc", "hsbc cc",
        "indusind cc",
...
def _derive_variant(name, institution, identifier):
    ...
    are already stored (e.g. 'HDFC-Tata Neu Infinity-8886' -> 'Tata Neu
...
def _confirm_merchant(conn, alias, category, canonical_hint, include_txn_id=None):
    ...
    if not alias:
        return None, 0
    # An alias this short is not a merchant name, it's debris — and because
    # matching is INSTR-based (substring, anywhere), a 2-character alias
    # hijacks every unrelated description that happens to contain those
    # letters. This is not hypothetical: a real 'cc' alias, left behind when
    # the normalizer reduced "UPI CC-04-01-2025-500476327499 (Ref# ...)" to
    # nothing but its rail prefix, silently claimed HSBC's "JOINING FEE
    # CC26201600663" and "IGST ASSESSMENT @18.00% CC26201600663" as
    # Reversals & Refunds — a fee and a tax filed as refunds.
    # Refused at the write boundary so no future confirm can create another.
