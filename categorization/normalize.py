...
# Shortest result normalize() will hand back as a real merchant identity.
# Below this, a stripped-down residual is a rail artifact, not a name — see
# MIN_MERCHANT_LEN's own note below for how this number was chosen.
MIN_MERCHANT_LEN = 3
...
def _strip_trailing(s: str) -> str:
    ...
    while changed:
        ...
        # trailing reference-code token: letters and digits glued into ONE
        # token (no internal separator) containing a 6+ digit run, e.g.
        # "dp015338113028vfnnd" or "bbpsdp2162317bj548z24foz". This is what a
        # bank/BBPS settlement rail glues onto a payment description, and it
        # is DIFFERENT per statement — normalizing it away is what lets
        # "BPPY CC PAYMENT <token>" collapse to one stable merchant instead of
        # a fresh one every cycle. Delimited by whitespace, '/', or '-' (the
        # three separators actually observed: "payment dp...", "payment/dp...",
        # "received - dp..."). Distinguished from a real merchant token (a
        # product code, a store number) by the 6-digit floor — no city or
        # brand name in this corpus's allow-lists carries that many digits.
        m = re.search(r"[\s/-]([a-z0-9]*\d{6,}[a-z0-9]*)$", s)
        if m:
            s = s[: m.start()]
            changed = True
            continue
...
def normalize(raw: str) -> str:
    ...
    if not re.search(r"[a-z]", s):
        return ""
    # A result shorter than MIN_MERCHANT_LEN is likewise treated as
    # unmappable — found the hard way: "UPI CC-04-01-2025-500476327499"
    # strips (correctly, by the existing trailing-number rule, one
    # dash-separated numeric segment at a time) all the way down to the bare
    # residual "cc", which has letters and so passed the check above, and got
    # confirmed as a real 2-letter "merchant" whose alias then matched anything
    # containing those two letters anywhere (an HSBC joining fee's own
    # reference number, among others — see app.py's MIN_ALIAS_LEN, the
    # write-time half of this same fix). 3 is not a guess: the real corpus's
    # shortest LEGITIMATE normalized merchants sit at exactly 4 characters
    # (amul, cred, cult, iocl, lulu, moai, savr) — 3 keeps every one of those
    # and a hypothetical 3-letter brand (kfc) while still catching "cc".
    if len(s) < MIN_MERCHANT_LEN:
        return ""
    return s
