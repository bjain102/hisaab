...
- ADR-004: Data-ingestion reality (what automates, what doesn't)
...
-- ── Net worth ──────────────────────────────────────────────────────────
CREATE TABLE valuations (                  -- dated value snapshots, all snapshot-valued kinds
  id          INTEGER PRIMARY KEY,
  account_id  INTEGER NOT NULL REFERENCES accounts(id),
  as_of       TEXT NOT NULL,
  value_paise INTEGER NOT NULL CHECK (value_paise >= 0),
  source      TEXT NOT NULL CHECK (source IN ('manual','kite','csv')),
  note        TEXT,                        -- "EPF passbook", "22g @ ₹7,100/g"
  created_at  TEXT NOT NULL DEFAULT (datetime('now')),
  UNIQUE (account_id, as_of)               -- same-day re-entry = correction (upsert)
);
CREATE INDEX idx_val_account_asof ON valuations(account_id, as_of);

CREATE TABLE holdings (                    -- per-instrument detail behind a Kite/CSV valuation
  id               INTEGER PRIMARY KEY,
  account_id       INTEGER NOT NULL REFERENCES accounts(id),
  as_of            TEXT NOT NULL,
  symbol           TEXT NOT NULL,          -- tradingsymbol / MF scheme
  isin             TEXT,
  name             TEXT,
  quantity         REAL NOT NULL,          -- MF units fractional; qty is not money → REAL is fine
  avg_cost_paise   INTEGER,                -- per unit
  last_price_paise INTEGER,                -- per unit
  value_paise      INTEGER NOT NULL,
  UNIQUE (account_id, as_of, symbol)
);

CREATE TABLE loans (                       -- formula-valued; 1:1 with accounts(kind='loan')
  account_id      INTEGER PRIMARY KEY REFERENCES accounts(id),
  principal_paise INTEGER NOT NULL,
  annual_rate_bps INTEGER NOT NULL,        -- 850 = 8.50%
...
CREATE TABLE loan_events (
  id           INTEGER PRIMARY KEY,
  account_id   INTEGER NOT NULL REFERENCES loans(account_id),
  date         TEXT NOT NULL,
  kind         TEXT NOT NULL CHECK (kind IN ('prepayment','rate_change')),
  amount_paise INTEGER,                    -- prepayment amount
  new_rate_bps INTEGER                     -- rate change
);
```
...
2. **v2 — paise:** add `amount_paise`; backfill `CAST(ROUND(amount*100) AS INTEGER)`; **verify per-row** `ABS(amount*100 − amount_paise) < 0.5` for all 602 rows and per-account sum equality; rebuild table without `amount` (SQLite can't drop-with-check; table rebuild per ADR-007).
...
9. **v9 — net-worth tables:** `valuations`, `holdings`, `loans`, `loan_events` (pure additions, no data migration).
...
## ADR-004 — Data-ingestion reality. **Accepted.**

Honest per-source assessment for one individual in India. **There is no consumer-grade aggregation API an individual can sign up for** — the Account Aggregator framework (Sahamati/Setu/Finvu) is real but gated behind FIU registration; it is B2B infrastructure, not a personal API. Anything promising otherwise is scraping with credentials, which this project will not do.

| Source | Channel | Automation level | Notes |
|---|---|---|---|
| Card transactions | PDF statements (+ Amex CSV) | **Semi-auto:** manual download+upload, automatic parse | The only reliable channel. Email auto-fetch rejected (IMAP + bank-mail scraping = fragility we just audited out). |
| Equity holdings | **Kite Connect API** | **One-click w/ login:** refresh button → Kite login → holdings pulled | **Pricing confirmed by owner (2026-07-12, Zerodha pricing page):** the Personal tier is free and includes portfolio management (holdings) — everything this app needs. The ₹500/mo Connect tier only adds WebSocket streaming and historical candle data, neither of which is used here. Access token expires daily at ~6 AM IST, which is irrelevant to a login-per-refresh flow. Redirect URL: `http://127.0.0.1:5000/api/kite/callback` (localhost redirects are permitted). API key/secret in gitignored `config.local.json`. |
| MF holdings (Coin) | Same Kite session, `mf/holdings` | Same as equity | Only covers Zerodha-held MF. External MF (if any exist) → manual valuation snapshot, or CAMS/KFintech CAS-PDF parsing as a *possible future parser* (explicitly not v1 — it's another fragile PDF format, and the corpus discipline of ADR-006 would apply). |
| Holdings fallback | Zerodha Console CSV export | Manual, ~2 min | Kept as insurance against API changes; imports as a snapshot with `source='csv'`. |
| EPF | Passbook portal, manual read | **Manual, monthly-ish** | No API. Passbook portal has no stable programmatic access; OTP-walled. |
| PPF | Netbanking balance, manual read | **Manual** | Same. |
| Gold | Manual (grams held × rate looked up, or flat value) | **Manual** | Rate APIs exist but add a dependency for a monthly 30-second task; note field records the math. |
| Property | Owner's estimate | **Manual, rare** | No honest API exists anywhere. |
| Bank balances | Manual snapshot | **Manual** | Balances-only per interview; txn import is a future door (ADR-003). |
| Loans | Terms entered once; balance **computed** | **Zero ongoing effort** | Amortization from terms; prepayments entered as events when made. |

Rejected alternatives: screen-scraping netbanking (credential risk, breaks monthly), email statement ingestion (fragile), paid aggregators (none sell to individuals), INDmoney/ET-Money export (no export APIs; their data is also aggregated via AA under *their* FIU license, unavailable to us).
