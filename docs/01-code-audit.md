...
**Date:** 2026-07-12
**Scope:** Full read of `app.py`, `pdf_parsers.py`, `debug_rewards.py`, templates/static, the live SQLite DB (`data/hisaab.db`, 602 transactions, Jan–Jul 2026), and the `statements/` folder (35 files). Parsers were executed against the redacted statement files. No application code was modified.
...
| Data | SQLite, 6 tables. 602 transactions across 8 card labels. Dates are clean (all `YYYY-MM-DD`), amounts currently all have ≤2 decimal places. |
...
- No indexes beyond PKs (harmless at 602 rows; noted for the net-worth tables which will be queried by `(account, date)` constantly).
...
- Redacted-file runs: 32/32 and 64/64 date-lines parsed, rewards found for both variants. Best-tested parser purely by usage volume (442 of 602 rows are HDFC).
...
- 602 transactions, 8 card labels, dates 2026-01-01 → 2026-07-01, all dates well-formed, `card_label` never null.
