...
- **Done:** all 602 rows reachable; filters combine; animated row stagger on filter change.
...
## Phase 6 — Net worth on manual data (M6, M7, M9-partial; ADR-003 v9)

**Ships:** real net worth from day one — without Kite. (Deliberate: manual assets prove the whole valuation pipeline with zero external dependencies.)

### 6.1 v9 — net-worth tables
- **Goal:** create `valuations`, `holdings`, `loans`, `loan_events` (pure DDL, no data).
- **Verify:** migration chain green on fixture + real DB; `./test.ps1`.

### 6.2 Accounts registry (M6)
- **Goal:** CRUD API + Assets page: add/edit/deactivate accounts of every kind incl. kind-specific `meta` fields; loan kind captures ADR-003 `loans` terms.
- **Verify:** create one account of each kind; restart app; all persist. Deactivate → excluded from net worth, history retained.

### 6.3 Manual valuations (M6)
- **Goal:** dated snapshot entry per account with staleness badges ("42 days old"); same-day re-entry upserts (correction semantics per schema `UNIQUE(account_id, as_of)`).
- **Verify:** `tests/test_networth.py` fixture: accounts with known snapshots at known dates → net worth at three dates matches hand-computed values exactly. UI: enter, edit, delete a snapshot; badge updates.

### 6.4 Amortization engine (M7)
- **Goal:** outstanding(t), EMI interest/principal split, prepayment re-basing, rate-change events — integer-paise math, round-half-up per period (ADR-005).
- **Files:** `networth/amortization.py` + tests.
- **Verify — acceptance gate:** synthetic loan ₹10,00,000 @ 9.00% / 60 months matches a bank-published schedule to the rupee at months 1, 12, 36, 60; a mid-tenure ₹1,00,000 prepayment reduces tenure per standard reducing-balance math.
- **Done when** the M7 spec test passes and a loan account shows outstanding + this-month interest in the UI.

### 6.5 Net Worth dashboard (M9, manual-only)
- **Goal:** total (animated ticker) + MoM delta, allocation by kind, stepped month-end timeline, liability-drag panel, per-account drill-down, "how is this computed" affordance, staleness carry-forward markers.
- **Verify:** against the 6.3 fixture: every panel's number traceable; a month with a missing snapshot shows carry-forward marker, not an interpolated value.

**Phase 6 exit:** the INDmoney-style view exists and is honest; equity/MF appear as manually-snapshotted accounts until Phase 7.

---

## Phase 7 — Kite integration (M8; ADR-004)

### 7.1 Kite Connect setup (owner task, documented)
- **Goal:** step-by-step doc: create app at developers.kite.trade on the free Personal tier (pricing confirmed — ADR-004), redirect URL `http://127.0.0.1:5000/api/kite/callback`, key/secret into gitignored `config.local.json`; `pip install kiteconnect` (pin it).
- **Verify:** `python -c "import kiteconnect"`; config loads; secrets absent from `git status`.

### 7.2 Refresh flow
- **Goal:** "Refresh from Kite" → login URL → callback exchanges request token → pull equity holdings + MF holdings → write `holdings` rows + one `valuations` row per account (`source='kite'`) atomically — cancelled/failed login leaves no partial snapshot.
- **Files:** `networth/kite.py`, callback route, button + status UI.
- **Verify:** live: one refresh → snapshot appears with instrument detail; cancel mid-login → no rows. Test: mocked Kite payloads → correct paise math (`avg_cost`, `last_price` arrive as float rupees from Kite; conversion rounding pinned in tests).

### 7.3 Console CSV fallback
- **Goal:** import Zerodha Console holdings CSV as a snapshot (`source='csv'`), same write path as 7.2.
- **Verify:** golden test on a synthetic Console-format CSV; UI import produces identical structure to an API snapshot.

### 7.4 Holdings detail + allocation completion
- **Goal:** per-instrument table under equity/MF accounts (qty, avg cost, LTP, value, day-ago delta if a prior snapshot exists); allocation panel now spans all kinds.
- **Verify:** allocation percentages sum to 100%; drill-down totals equal the account valuation to the paisa.

**Phase 7 exit:** the full spec is live. ~25 min/month sustains everything.
