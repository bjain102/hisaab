...
def test_confirm_refuses_a_dangerously_short_alias(client):
    """Alias matching is INSTR-based, so a 2-character alias is a wildcard, not
    a narrow rule. A real 'cc' alias — debris left when the normalizer reduced
    "UPI CC-04-01-2025-500476327499 (Ref# ...)" to just its rail prefix —
    silently claimed an HSBC statement's "JOINING FEE CC26201600663" and
    "IGST ASSESSMENT @18.00%" rows as Reversals & Refunds: a fee and a tax
    filed as refunds. Refused at the write boundary now."""
    before = len(client.get('/api/merchants').get_json())
    resp = client.post('/api/review_queue/confirm',
                       json={'merchant': 'cc', 'category': 'Food & Drinks'})
    # Rejected or a no-op, but never a stored 2-char alias.
    aliases = [m.get('alias') or '' for m in client.get('/api/merchants').get_json()]
    assert 'cc' not in aliases
    assert len(client.get('/api/merchants').get_json()) == before
