...
def test_variant_derived_correctly_for_multiword_variant(client):
    client.post('/api/card_profiles', json={'bank': 'hdfc', 'variant': 'Tata Neu Infinity', 'last4': '8886'})
    profiles = client.get('/api/card_profiles').get_json()
    row = next(p for p in profiles if p['last4'] == '8886')
    assert row['variant'] == 'Tata Neu Infinity'
    assert row['label'] == 'HDFC-Tata Neu Infinity-8886'
